...
## Resume Generation (JSON->LaTeX; applications live in a PRIVATE repo)
Resumes are generated from Pranay's polished JSON->LaTeX system, **not** the legacy moderncv template in `cv/`.

- **PRIVACY (important):** per-application packages contain defense-manufacturing detail and internal strategy notes (fit scores, honesty flags, rejection analysis). They live in a **private** repo, `/home/pranay/projects/job-applications/`. **Never commit application packages, notes, or the RESL achievement detail to a public repo.** The public engine repo (`/home/pranay/projects/resume`, `KiranPranay/XeLaTeX-Spidy-Resume`) and this `ai-job-search` fork are both public - keep only resume/LinkedIn-level content in them.
- **Engine:** template `templates/spidy.tex` (single-column IBM Plex Sans) + `render.py` (stdlib-only, substitutes `<<TOKENS>>` from a data JSON) + `latexmk`. The private repo self-contains a copy of `render.py` + `templates/spidy.tex`, so it builds standalone.
- **Master data:** `job-applications/data/re.json` (working master for application tailoring; the public resume repo keeps its own `data/re.json` for the public showcase resume - sync manually if the baseline changes). Tailor a *copy* per application; never fabricate beyond the profile. Detailed quantified achievements come from the private achievement bank (Claude memory), not the public profile.
- **Per-application folder:** `job-applications/applications/<company>_<role-slug>/` containing `resume.json` (tailored data), `config.json` (`outdir` = this folder), `job.md` (the posting text + an `**Apply:**` URL), `notes.md` (posting URL, fit eval, requirement coverage, tailoring + honesty notes, status), `interview.md` (role-specific interview prep), the built `Pranay_Kiran_Resume.{tex,pdf}` and `Pranay_Kiran_CoverLetter.{tex,pdf}` (**company-neutral filenames** - recruiters see the uploaded filename, so never suffix it with the company; the folder identifies the application; cover letter authored in this ai_job repo under `cover_letters/` which is gitignored, PDF copied in), and a generated `index.html` (self-contained interactive briefing: job post + notes + interview + résumé + cover, tabbed, theme-aware, with a prominent Apply link).
- **Build:** from the job-applications repo root, `./applications/build.sh <slug>` (renders + compiles + cleans, then regenerates `index.html` via `applications/make_index.py` and the tracker via `applications/make_tracker.py`).
- **Bullet order:** `render.py` emits a role's `impact` list before its `responsibilities`; to control order, put bullets in `impact`.
- **Tracker:** `job-applications/applications/index.html` is the master dashboard across all applications (status, fit, confirmations pending, next actions), generated from each package's `notes.md`. When an application is submitted or gets a response, update the `- **Status:**` line in that package's `notes.md` and regenerate.
- Full convention: `job-applications/applications/README.md`.
